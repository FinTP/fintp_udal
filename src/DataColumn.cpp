...
{
	DEBUG2( "Base get long" );
	// Ugly Oracle hack
	try
	{
		DEBUG2( "Attempt cast to long column" );
		DataColumn< long > * castColumn = ( dynamic_cast< DataColumn< long > * >( this ) );
		if( castColumn == NULL )
			throw runtime_error( "invalid cast" );

		return castColumn->getValue();
	}
	catch( ... )
	{
		TRACE( "Attempted cast to DataColumn< long > on a non long column and failed. Falling back to parse from string ... " );
		DataColumn< string > * castColumn = ( dynamic_cast< DataColumn< string > * >( this ) );
		if ( castColumn == NULL )
			throw runtime_error( "invalid cast" );

		return StringUtil::ParseLong( castColumn->getValue() );
	}
}
